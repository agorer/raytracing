...
let rec build_color i j world pixel00_loc delta_u delta_v origin samples_left scale color_sum =
  let open Vec3d in
  match samples_left with
  | 0 -> color_sum * scale
  | samples_left ->
    let ray = get_ray i j pixel00_loc delta_u delta_v origin in
    let color_sum = color_sum + (ray_color ray world) in
    let samples_left = Stdlib.(samples_left - 1) in
    build_color i j world pixel00_loc delta_u delta_v origin samples_left scale color_sum
    ...
    for i = 0 to Stdlib.(camera.image_width - 1) do
      ...
      let pixel_color =
        build_color i j world
          ...
          camera.samples_per_pixel pixel_samples_scale (0., 0., 0.) in
